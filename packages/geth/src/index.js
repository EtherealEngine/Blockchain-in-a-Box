...
const accountMainnetJson = JSON.parse(
  fs.readFileSync("./account-mainnet.json", "utf8")
);
const accountRinkebyJson = JSON.parse(
  fs.readFileSync("./account-rinkeby.json", "utf8")
);

const networkInterfaces = os.networkInterfaces();
const eth0Interface = networkInterfaces.eth0.find(
  (spec) => spec.family === "IPv4"
);
...
const cpSidechain = childProcess.spawn(
  "geth",
  [
    "--datadir",
    "rinkeby",
...
cpSidechain.stdout.pipe(
  fs.createWriteStream("./rinkeby-stdout.log", {
    flags: "a",
  })
);
cpSidechain.stderr.pipe(
  fs.createWriteStream("./rinkeby-stderr.log", {
    flags: "a",
  })
);
...
const cpMainnet = childProcess.spawn(
  "geth",
  [
    "--datadir",
    "mainnet",
...
cpMainnet.stdout.pipe(
  fs.createWriteStream("./mainnet-stdout.log", {
    flags: "a",
  })
);
cpMainnet.stderr.pipe(
  fs.createWriteStream("./mainnet-stderr.log", {
    flags: "a",
  })
);

const childProcesses = [cpSidechain, cpMainnet];
