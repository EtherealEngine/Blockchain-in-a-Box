    ...
    client.on("messageReactionAdd", async (reaction, user) => {
      ...
      if (user.id !== client.user.id && emoji.identifier === "%E2%9D%8C") {
        ...
        if (message.channel.type === "dm") {
          ...
          if (
            (helpIndex = helps.findIndex((help) => help.id === message.id)) !==
            -1
          ) {
            ...
            (trade = trades.find((trade) => trade.id === message.id)) &&
        ...
        // left arrow ◀️
        if (
          (inventoryIndex = inventories.findIndex(
            (i) => i.id === message.id
          )) !== -1
        ) {
      ...
      if (!message.author.bot) {
        ...
        if (message.channel.type === "text") {
          ...
          const _items = (contractName, page) => async (getEntries) => {
            ...
            if (
              split.length >= 2 &&
              (match = split[1].match(/<@!?([0-9]+)>/))
            ) {
              await _loadFromUserId(match[1]);
            } else if (
              split.length >= 2 &&
              (match = split[1].match(/^(0x[0-9a-f]+)$/i))
            ) {
            ...
            if (split[0] === prefix + "help") {
              ...
              if (!name) {
                ...
                if (command) {
                  ...
                  const helpField =
                    helpFields.find((hf) => hf.shortname === name) ||
                    helpFields.find((hf) => hf.shortname === "help");
                  exampleEmbed.addFields([helpField]);
                }
              }
              const m = await message.channel.send(exampleEmbed);
              m.react("❌");
              ...
              const [
                ...
                contracts.Account.methods.getMetadata(address, "name").call(),
                contracts.Account.methods
                  .getMetadata(address, "description")
                  .call(),
                contracts.Account.methods
              ...
              const token = await contracts.NFT.methods
                .tokenByIdFull(tokenId)
                .call();
              // console.log('got token', token);
              const minterAddress = token.minter.toLowerCase();
              const [unlockable, minterName, minterAvatarPreview] =
                await Promise.all([
                  (async () => {
                    const key = unlockableKey;
                    const value = await contracts.NFT.methods
                      .getMetadata(token.hash, key)
                      .call();
                    return !!value;
                  })(),
                  contracts.Account.methods
                    .getMetadata(minterAddress, "name")
                    .call(),
                  contracts.Account.methods
              ...
              const exampleEmbed = new Discord.MessageEmbed()
                ...
                .setAuthor(
                  minterName || "Anonymous",
                  minterAvatarPreview,
                  `https://webaverse.com/accounts/${minterAddress}`
                )
                ...
                .addFields(
                  ...
                  { name: "file size", value: sizeString + " (est.)" },
                  {
                    ...
                    value: resolutionString + " (est.)",
                  },
                  {
                    name: "collaborators",
                    value: collaborators.join(", ") + " (est.)",
                  },
              ...
              if (split[1]) {
                ...
                message.channel.send(
                  "<@!" +
                    message.author.id +
                    ">: set monetization pointer to " +
                    JSON.stringify(monetizationPointer)
                );
              } else {
                const wallet = hdkey
                  .fromMasterSeed(bip39.mnemonicToSeedSync(mnemonic))
              ...
              async function getLoadout(address) {
                const loadoutString = await contracts.Account.methods
              ...
              if (index >= 1 && index <= 8) {
                ...
                if (!isNaN(id)) {
                  ...
                  message.channel.send(
                    "<@!" + message.author.id + ">: updated loadout"
                  );
                } else {
              ...
              if (!isNaN(id)) {
                ...
                message.channel.send(
                  "<@!" + message.author.id + ">: set home space to " + id
                );
              } else {
                const wallet = hdkey
                  .fromMasterSeed(bip39.mnemonicToSeedSync(mnemonic))
                  ...
                  .getWallet();
                const address = wallet.getAddressString();
                const homeSpaceUrl = await contracts.Account.methods
              ...
              const signature = await contracts.Account.methods
              ...
              if (signature !== "") {
                ...
                message.channel.send(
                  "<@!" + message.author.id + ">: no role redeemed."
                );
                return;
              }
              ...
              for (let i = 0; i < nftMainnetBalance; i++) {
                ...
                if (
                  token.id >= genesisNftStartId &&
                  token.id <= genesisNftEndId
                ) {
                  const genesisRole = message.guild.roles.cache.find(
              ...
              if (
                ...
                if (balance === "0") {
                  message.channel.send(
                    "<@!" +
                      ...
                      "> has " +
                      balance +
                      " SILK. Want to get some SILK? Ask the Webaverse team: https://discord.gg/R5wqYhvv53"
                  );
                } else {
                  message.channel.send(
                ...
                if (balance === "0") {
                  ...
                  message.channel.send(
                    "<@!" + message.author.id + "> has " + balance + " SILK."
                  );
                }
              }
              ...
              if (split[1] !== "treasury") {
                if (split[1] && (match = split[1].match(/<@!?([0-9]+)>/))) {
                  const userId = match[1];
                  const member = await message.channel.guild.members.fetch(
                    userId
                  );
                ...
                if (user) {
                  ...
                  if (userId !== message.author.id) {
                    ...
                    const treasurer = member.roles.cache.some(
                      (role) => role.name === treasurerRoleName
                    );
                    if (treasurer) {
                      ...
                      message.channel.send(
                        "<@!" + message.author.id + ">: you are not a treasurer"
                      );
                      return;
                    }
                  }
              ...
              if ((match = split[1].match(/<@!?([0-9]+)>/))) {
                ...
                if (user) {
                  ...
                  const headerLeft =
                    ...
                    tradeId +
                    " " +
                    message.author.username.padStart(8);
                  const headerMiddle = " | ";
                  ...
                  m.finish = async () => {
                    ...
                    message.channel.send(
                      "```trade #" + tradeId + " complete! enjoy!```"
                    );
                  };
                  trades.push(m);
                } else {
                  message.channel.send(
                    "<@!" + message.author.id + ">: cannot find peer"
                  );
                }
              } else {
                message.channel.send(
              ...
              if (!isNaN(price)) {
                ...
                const ownTokenIds = [];
                const nftBalance = await contracts.NFT.methods
                  .balanceOf(address)
                  .call();
                ...
                if (treasurer) {
                  ...
                  for (let i = 0; i < nftBalance; i++) {
                    const id = await contracts.NFT.methods
                      .tokenOfOwnerByIndex(treasuryAddress, i)
                      .call();
                ...
                if (ownTokenIds.includes(tokenId)) {
                  ...
                  try {
                    ...
                    const buySpec = await runSidechainTransaction(
                      treasuryMnemonic
                  ...
                  message.channel.send(
                    "<@!" + message.author.id + ">: not your nft: " + tokenId
                  );
                }
              } else {
                message.channel.send(
                  "<@!" + message.author.id + ">: invalid price: " + split[2]
                );
              }
              ...
              if (!isNaN(buyId)) {
                ...
                if (status) {
                  message.channel.send(
                    "<@!" + message.author.id + ">: unlisted sell " + buyId
                  );
                } else {
                  message.channel.send(
                    "<@!" + message.author.id + ">: unlist failed: " + buyId
                  );
                }
              } else {
                message.channel.send(
                  "<@!" + message.author.id + ">: invalid sell id: " + split[1]
                );
              }
              ...
              try {
                ...
                const store = await contracts.Trade.methods
                ...
                if (entries.length > 0) {
                  ...
                  s +=
                    ...
                    entries
                      .map(
                        (entry, i) =>
                          `${entry.id}. ${entry.name} -> ${entry.hash}`
                      )
                      .join("\n") +
                    "```";
                } else {
              ...
              if (!isNaN(tokenId)) {
                ...
                if (status) {
                  message.channel.send(
                    "<@!" + message.author.id + ">: deploy successful, sarge!"
                  );
                } else {
                  message.channel.send(
                    "<@!" + message.author.id + ">: deploy failed"
                  );
                }
              } else {
                message.channel.send(
                  "<@!" + message.author.id + ">: invalid land nft: " + split[1]
                );
              }
            } else if (
              split[0] === prefix + "landcollab" &&
              split.length >= 3
            ) {
                ...
                if (index >= 0) {
                  ...
                  if (!trade.nfts[index].includes(id)) {
                    ...
                    if (hash) {
                      ...
                      const balance = await contracts.NFT.methods
                      ...
                      if (balance > 0) {
                        if (trade.nfts[index].length < 3) {
                          ...
                          const doneReactions = trade.reactions.cache.filter(
                            (reaction) =>
                              reaction.emoji.identifier === "%E2%9C%85"
                          );
                          try {
                            for (const reaction of doneReactions.values()) {
                              const users = Array.from(
                                reaction.users.cache.values()
                              );
                              for (const user of users) {
                        ...
                        message.channel.send(
                          "<@!" + message.author.id + ">: not your nft: " + id
                        );
                      }
                    } else {
                      message.channel.send(
                        "<@!" + message.author.id + ">: invalid nft: " + id
                      );
                    }
                  } else {
                    message.channel.send(
                  ...
                  message.channel.send(
                    "<@!" + message.author.id + ">: not your trade: " + split[1]
                  );
                }
              } else {
                message.channel.send(
                  "<@!" + message.author.id + ">: invalid trade: " + split[1]
                );
              }
                ...
                if (packedBalances.length > 0) {
                  s +=
                    ...
                    packedBalances
                      .map(
                        (pack, i) =>
                          `${pack.id}. contains ${pack.packedBalance} FT`
                      )
                      .join("\n") +
                    "```";
                } else {
              ...
              if (!isNaN(tokenId) && !isNaN(amount)) {
                ...
                message.channel.send(
                  "<@!" + message.author.id + ">: invalid token id: " + split[1]
                );
              }
              ...
              if (!isNaN(id)) {
                ...
                if (!isNaN(quantity)) {
                  if ((match = split[1].match(/<@!?([0-9]+)>/))) {
                      ...
                      try {
                        ...
                        for (let i = 0; i < nftBalance; i++) {
                          const id = await contracts.NFT.methods
                            .tokenOfOwnerByIndex(address, i)
                            .call();
                          const hash2 = await contracts.NFT.methods
                            .getHash(id)
                            .call();
                          if (hash2 === hash) {
                            ids.push(id);
                        ...
                        if (ids.length >= quantity) {
                          ...
                          for (let i = 0; i < quantity; i++) {
                            const id = ids[i];
                            const result = await runSidechainTransaction(
                              mnemonic
                            )("NFT", "transferFrom", address, address2, id);
                    ...
                    const wallet2 = hdkey
                      .fromMasterSeed(
                        bip39.mnemonicToSeedSync(treasuryMnemonic)
                      )
                      .derivePath(`m/44'/60'/0'/0/0`)
                      .getWallet();
                    const address2 = wallet2.getAddressString();
                    ...
                    if (status) {
                      ...
                      message.channel.send(
                        "<@!" + message.author.id + ">: could not transfer"
                      );
                    }
                  } else {
                    message.channel.send("unknown user");
                ...
                message.channel.send(
                  "<@!" + message.author.id + ">: invalid token id: " + split[2]
                );
              }
                ...
                return (
                  new Discord.MessageEmbed()
                    ...
                    .setAuthor(
                      userName,
                      avatarPreview,
                      `https://webaverse.com/accounts/${address}`
                    )
                    // .setDescription(description || 'This person is a noob without a description.')
                    // .setThumbnail(avatarPreview)
                    // .addField('Inline field title', 'Some value here', true)
              ...
              if (!isNaN(id)) {
                ...
                message.channel.send(
                  "<@!" + message.author.id + ">: invalid token id: " + id
                );
              }
            } else if (
              split[0] === prefix + "preview" &&
              split.length >= 2 &&
              !isNaN(parseInt(split[1], 10))
            ) {
              const id = parseInt(split[1], 10);
              ...
              const hash = await contracts.NFT.methods.getHash(id).call();
              const ext = await contracts.NFT.methods
              ...
              if (currentName) {
                ...
                if (isNaN(realmId)) {
                  message.channel.send(
                    "<@!" + message.author.id + ">: must add realm id. (1-5)"
                  );
                } else {
              ...
              const [
                ...
                (async () => {
                  const isC = await contracts.NFT.methods
              ...
              if (isC || isO) {
                let value = await contracts.NFT.methods
                  .getMetadata(hash, key)
                  .call();
              ...
              if (split[0] === prefix + "mint") {
                ...
                if (files.length > 0) {
                            ...
                            if (status) {
                              message.channel.send(
                                "<@!" +
                                  ...
                                  (tokenIds[0] === tokenIds[1]
                                    ? "https://webaverse.com/assets/" +
                                      tokenIds[0]
                                    : tokenIds
                                        .map(
                                          (n) =>
                                            "https://webaverse.com/assets/" + n
                                        )
                                        .join(" - ")) +
                                  " (" +
                                  hash +
                              ...
                              if (balance < 10) {
                                message.channel.send(
                                  "<@!" +
                                    message.author.id +
                                    ">: mint transaction failed: you do not have enough SILK. Ask the Webaverse team for some! https://discord.gg/R5wqYhvv53"
                                );
                              } else {
                                message.channel.send(
                                  "<@!" +
                  ...
                  message.channel.send(
                    "<@!" + message.author.id + ">: no files to mint"
                  );
                }
                ...
                if (files.length > 0) {
                  const oldHash = await contracts.NFT.methods
                    .getHash(tokenId)
                    .call();
                  ...
                  message.channel.send(
                    "<@!" + message.author.id + ">: no files to update"
                  );
                }
              }
            }
            ...
            if (split.length === 1 + 12) {
              ...
              if (bip39.validateMnemonic(mnemonic)) {
                await ddb
                  ...
                  .promise();
                message.author.send(
                  "set key to ```" + JSON.stringify(mnemonic) + "```"
                );
              } else {
