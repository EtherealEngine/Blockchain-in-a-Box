export default ({ IDL }) => {
  ...
  const WicpTxReceipt = IDL.Variant({ 'Ok' : IDL.Nat, 'Err' : WicpTxError });
  const BuyResult = IDL.Variant({ 'Ok' : WicpTxReceipt, 'Err' : IDL.Text });
  const ListResult = IDL.Variant({ 'Ok' : IDL.Bool, 'Err' : IDL.Text });
  ...
  const erc721_token = IDL.Service({
    ...
    'buyDip721' : IDL.Func([IDL.Nat64], [BuyResult], []),
    'delistFromSale' : IDL.Func([IDL.Nat64], [ListResult], []),
    ...
    'listForSale' : IDL.Func([IDL.Nat64, IDL.Nat], [ListResult], []),
...
export const init = ({ IDL }) => {
  return [IDL.Principal, IDL.Text, IDL.Text, IDL.Principal, IDL.Principal];
};
